...
const Sidebar = () => {
  ...
  return (
    <aside
      className="flex flex-col flex-shrink-0 bg-white border-r border-slate-200 h-full relative"
      style={{
    ...
    >
      <div className={`flex items-center gap-2.5 ${isSidebarCollapsed ? 'justify-center pt-5 pb-4 px-0' : 'justify-start pt-5 pb-4 px-4'}`}>
        <div
          className="flex-shrink-0 cursor-pointer text-indigo-500 hover:text-indigo-600 transition-colors"
        ...
        {!isSidebarCollapsed && (
          <div className="flex flex-col">
            <span className="text-[15px] font-bold text-slate-900 tracking-tight font-sans">Repo Into Graph</span>
      ...
      <nav className="flex-1 flex flex-col gap-1 px-2.5 py-2">
        {!isWorkspace ? (
          ...
          <>
            <button
              className={`flex items-center gap-2 px-3 py-2.5 rounded-[var(--radius-sm)] transition-colors w-full text-left font-sans text-[13px] font-medium text-slate-500 hover:bg-slate-50 hover:text-slate-800 ${isSidebarCollapsed ? 'justify-center py-2.5 px-0' : 'justify-start'}`}
      ...
      <div className={`border-t border-slate-200 flex items-center ${isSidebarCollapsed ? 'flex-col p-3 gap-3' : 'flex-row px-4 py-3 gap-2'}`}>
        <div className={`flex items-center gap-2 ${isSidebarCollapsed ? 'justify-center' : 'justify-start flex-1'}`}>
          ...
          {!isSidebarCollapsed && <span className="text-[11px] text-slate-500 font-medium truncate">{serverUrl}</span>}
        </div>
        <button
          className="w-7 h-7 rounded bg-transparent border-none text-slate-400 cursor-pointer flex items-center justify-center transition-colors hover:bg-slate-100 hover:text-slate-700"
          onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
